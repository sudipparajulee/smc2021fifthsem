...
<form action="{{route('notice.update',$notice->id)}}" method="POST" class="mt-5">
    @csrf
    <input type="date" class="block w-full my-5 rounded-lg border-gray-300" placeholder="Notice Date" name="notice_date" value="{{$notice->notice_date}}">
    @error('notice_date')
        <span class="text-red-600 text-sm block -mt-5">{{$message}}</span>
    @enderror
    <input type="text" class="block w-full my-5 rounded-lg border-gray-300" placeholder="Enter Notice" name="title" value="{{$notice->title}}">
    @error('title')
        <span class="text-red-600 text-sm block -mt-5">{{$message}}</span>
    @enderror
